...
public class EditIssueActivity extends ActionBarActivity implements IChangeDateListener {
	...
	SelectedIssuesFolder getSelectedFolder() {
		if (m_currentFolder.size() > 0 && m_currentFolder.get(0).getProject()!=null) {
	...
	void setIssuesFolder(String connectionId, Long id) {
		...
		if (id == null || id.equals(0L)) {
			...
			MDataHelper hlp = new MDataHelper(getApplicationContext(), connectionId);
	...
	private void updateMoreSettingsCombo() {
		...
		cmbMoreSettings.setVisibility(arrMoreSettings.size()>1?View.VISIBLE:View.GONE);
		...
		cmbMoreSettings.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				cmbMoreSettings.setVisibility( cmbMoreSettings.getAdapter().getCount()>1?View.VISIBLE:View.GONE);
			}
	...
	private void setDueDate(Calendar dueDate) {
		...
		if (dueDate == null) {
			...
			arr.add(Time.formatDate(getApplicationContext(), m_dueDate.getTimeInMillis()));
			btnClearDueDate.setVisibility(View.VISIBLE);			
			lblDue.setVisibility(View.VISIBLE);
		}
		
		ArrayAdapter<CharSequence> adapter = new ArrayAdapter<CharSequence>(getBaseContext(), 
	...
	private void createUi() {
		...
		Button btn2 = (Button)findViewById(R.id.btnButton2);
		if (m_details == null) {
			btn1.setText(getString(R.string.cancel));
			btn2.setText(getString(R.string.ok));
			...
			/*
			1. local: save, complete
			2. remote non synced: save, delete
			*/
			if (m_details instanceof MLocalIssue) {
				btn1.setText(m_details.getState()==State.TERMINATED?getString(R.string.resume):getString(R.string.completed));
				btn2.setText(getString(R.string.save));
				btn1.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						taskCompleted();					
		...
		inputMethodManager.showSoftInput(hours, InputMethodManager.SHOW_IMPLICIT);
		inputMethodManager.showSoftInput(description, InputMethodManager.SHOW_IMPLICIT);
		//////////////////////////////////////////////////////////
		// Priority
		//
		
		m_priorities.add(getString(R.string.issue_priority, getString(R.string.priority_low)));
		m_priorities.add(getString(R.string.issue_priority, getString(R.string.priority_normal)));
		m_priorities.add(getString(R.string.issue_priority, getString(R.string.priority_high)));
		m_priorities.add(getString(R.string.issue_priority, getString(R.string.priority_blocker)));
		
		Spinner cmbPriority = (Spinner) findViewById(R.id.cmbPriority);
		ArrayAdapter<CharSequence> priorityAdapter = new ArrayAdapter<CharSequence>(getBaseContext(), 
				android.R.layout.simple_spinner_item, m_priorities);
			...
			public boolean onTouch(View v, MotionEvent event) {
				if (event.getAction()==MotionEvent.ACTION_UP) {
					...
					dialog.setListener(new FolderListener() {						
						...
						@Override
						public void onFolderSelected(String connectionId, Long projectId, Long folderId) {
		...
		if (m_details != null) {
			...
			if (m_details instanceof MRemoteNotSyncedIssue) {
				...
				if (rnsi.getFailure()!=null) {
					...
					lblFailure.setText(getString(R.string.sync_error, rnsi.getFailure()));
				}				
			}
			...
			// priority
			int priority = m_details.getPriority()==null?Priority.NORMAL:m_details.getPriority();			
			switch (priority) {
			case Priority.LOW:
			...
			if (m_details instanceof MLocalIssue) 
				...
				throw new RuntimeException("View doesn't support this kind of Issue");
				
			
			// due date 
			if (m_details.getDueDate()!=null && m_details.getDueDate()>0) {
				Calendar c = Calendar.getInstance();
				c.setTimeInMillis(m_details.getDueDate());
				onDateSet(c.get(Calendar.YEAR), c.get(Calendar.MONTH), c.get(Calendar.DAY_OF_MONTH));
			...
			if (m_details instanceof MRemoteNotSyncedIssue) {
				MRemoteNotSyncedIssue rnsi = (MRemoteNotSyncedIssue)m_details;
				if (rnsi.getAssigneeId()!=null && rnsi.getAssigneeId()>0) {
					setAssignee(rnsi.getAssigneeId());
					findViewById(R.id.cmbAssignee).setVisibility(View.VISIBLE);
				}
			} 
			...
			if (m_details instanceof MRemoteNotSyncedIssue) {
				MRemoteNotSyncedIssue rnsi = (MRemoteNotSyncedIssue)m_details;
				if (rnsi.getResponsibleId()!=null && rnsi.getResponsibleId()>0) {
					setResponsible(rnsi.getResponsibleId());
					findViewById(R.id.cmbResponsible).setVisibility(View.VISIBLE);
				}
			} 
			...
			if (m_details.getDescription() != null) {
				...
				findViewById(R.id.edtDescription).setVisibility(View.VISIBLE);
			} 
		}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().getExtras()!=null)
			m_details = (MIssue)getIntent().getExtras().get(MIssue.class.getName());
		else
			m_details = null;
	...
	private MIssue validateInput() {
		...
		MIssue t = (folderId!=null && folderId != 0L)?new MRemoteNotSyncedIssue():new MLocalIssue();
		...
		if (t instanceof MRemoteNotSyncedIssue)
			((MRemoteNotSyncedIssue)t).setFolderId(cmbFolder.getSelectedItemId());
		...
		if (pnlBudget.getVisibility() == View.VISIBLE) {
			EditText hours = (EditText) findViewById(R.id.edtHours);
			EditText minutes = (EditText) findViewById(R.id.edtMinutes);
			t.setBudget(Integer.valueOf(Time.getMinutes(hours.getText().toString(), minutes.getText().toString())));
		} else 
			t.setBudget(null);
		...
		if (t instanceof MRemoteNotSyncedIssue) {
			...
			if (cmbAssignee.getVisibility() == View.VISIBLE) {
				if (m_assignee!=null) 
					((MRemoteNotSyncedIssue)t).setAssigneeId(m_assignee.getId());
				else
		...
		if (t instanceof MRemoteNotSyncedIssue) {
			...
			if (cmbResponsible.getVisibility() == View.VISIBLE) {
				if (m_responsible != null) 
					((MRemoteNotSyncedIssue)t).setResponsibleId(m_responsible.getId());
				else
		...
		if (m_details==null) {
			...
			if (t instanceof MRemoteNotSyncedIssue) {
				((MRemoteNotSyncedIssue)t).setSrvConnId(getSelectedFolder().getConnectionId());
			}
		}
		...
		.setPositiveButton(R.string.yes,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog,
							int id) {
						if (m_details instanceof MLocalIssue) 
							BL.getLocal(getApplicationContext()).deleteTask((MLocalIssue)m_details);
						else if (m_details instanceof MRemoteNotSyncedIssue) 
							BL.getLocal(getApplicationContext()).deleteTask((MRemoteNotSyncedIssue)m_details);
						setResult(RESULT_OK);
						finish();
					}
	...
	void taskCompleted() {
		new AlertDialog.Builder(this)
		.setMessage(m_details.getState()==State.TERMINATED? R.string.resume_task_confirm:R.string.close_task_confirm)
		.setCancelable(false)
		.setPositiveButton(R.string.yes,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog,
							int id) {
						if (m_details instanceof MLocalIssue) 
							BL.getLocal(getApplicationContext()).completeTask((MLocalIssue)m_details);
						setResult(RESULT_OK);
						finish();
					}
				}).setNegativeButton(R.string.no, null).show();
	...
	void cancelEditing() {
		new AlertDialog.Builder(this)
		.setMessage(m_details==null?R.string.cancel_creating_task_confirm:R.string.cancel_editing_task_confirm)
		.setCancelable(false)
		.setPositiveButton(R.string.yes,
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog,
